...
[TIP]
Speaking of working with images, Blender has an add on that you can install called "Insert Images as Planes" that is a bit more versatile than "Add Image Background" option. The ADD menu will now have this option available.  It will assign the material of the image to the plane and allow you to rely on things like ALPHA masking and is affected by scene lighting, but like I mentioned above, it will only show up in a TEXTURE view mode, so is not usefull for a background refrence drawing. 
...
[TIP]
Blender 3.0 has a new Asset Library feature where you can store pre-made objects and textures that are shared among your projects.  A perfect place to store various pre-made Bogies, Couplers and other common objects.
...
Zoom:: Select the default cube and press and hold kbd:[MMB] and then press the kbd:[CTRL] key. Moving up, down will zoom the views in and out.
...
[TIP]
If you have not already done so, you can also go to the `SYSTEM` section in `Preferences` and increase your number of UNDO steps to 200 or more.  Under `Themes->3d ViewPort`, you can scroll to the bottom and make your vertex sizes larger so they can be more easily seen. I normally use 5 for Vertex Size and `Face Dot Size`.  You can also adjust the color of the selected object and active object to make them stand out more.

[IMPORTANT]
You will also want to download and unzip the "Blender 2.8 to MSTS Exporter" from Elvas Tower and then read the instructions on how to install and use it. It includes very fine PDF file with instructions on how to use it.
...
[TIP]
Since I edited my copy of the MSTS/ORTS export ADD-ON to have an export as `.DDS` option as weall as `.ACE`, my `.S` file has the following in it:
...
This section mainly appies to users who still use a version of Blender prior to version 3.0.
...
[TIP]
Erick Cantu has supplied a number of truck and coupler shapes at Elvas Tower website, for example.  I have uploaded a usable Blender example there as well.
